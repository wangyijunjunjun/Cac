	...
	var KeyBoard = function (input, options) {
		...
		function addEvent(e) {
			...
			if (clickEl.tagName.toLocaleLowerCase() === 'td' && value !== "删除") {
				...
				//点击完成,计算
				if(input == storage_input){
					//time_input.value = "245";
					//alert(input.value);
					//alert(Number(input.value) * 1024 / bitrate/ 60 / 60);
					cacTime(input.value,unit_div.innerHTML);
				}else if(input ==time_input){
					cacStorage(input.value,unit_div.innerHTML);
				}
